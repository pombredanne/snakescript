...
MODULE_TEMPLATE = """/* {imports} */
$module("{name}", function() {{ with ({name}) {{
...
def _stmt(a):
    if isinstance(a, _ast.FunctionDef):
        ...
        if len(a.targets) == 1:
            if isinstance(a.targets[0], _ast.Name):
                script = "var {0} = {1};".format(
                    _expr(a.targets[0]),
                    _expr(a.value)
                )
            else:
        ...
        if a.orelse:
            if len(a.orelse) == 1 and isinstance(a.orelse[0], _ast.If):
                script += "\nelse " + _stmt(a.orelse[0])
            else:
